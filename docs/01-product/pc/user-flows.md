...
1. User starts dsh-work.
2. dsh-work loads its Configured Run context—DSH runtime, DSH data directory and
...
5. dsh-work starts the Worker and waits for a validated readiness signal.
6. On success, dsh-work navigates the embedded view to the DSH Workspace surface
   through the loopback Worker URL. DSH resumes the current Workspace or asks
   the user to select or create one; dsh-work then enters `Ready` for that
   Workspace context.
7. On failure, dsh-work shows a stable error code, plain-language cause and safe next actions.

The user never needs to choose a port, inspect a terminal or configure a
Workspace path in dsh-work's global settings. dsh-work never silently turns its
...
1. Closing the window applies the close-to-tray setting; by default it hides
   the window while dsh-work remains available in the system tray.
...
## UF-02A — Manage dsh-work settings

1. User opens `Settings` from the application menu or system tray.
2. dsh-work opens the separate trusted Settings window on its flat top-level
...
3. User changes the DSH runtime, DSH data directory or profile in the Run
   context. After a complete valid context is selected, dsh-work immediately
...
1. dsh-work shows the current Run context and its `Ready`, `Starting`, `Stopping`
   or `Failed` state.
...
3. dsh-work validates the complete candidate triple, enters `Stopping`, blocks
   context and plugin mutations, and stops the current Worker generation.
4. dsh-work verifies cleanup, starts the candidate as a new generation and keeps
   the previous context as the known-good rollback context.
5. Only after the candidate reaches `Ready` does dsh-work commit it as the current
   and Configured Run context and open the DSH Workspace surface for that
   generation.
6. If the candidate fails, dsh-work discards it and automatically restarts the
   known-good context. The failed candidate is never presented as current and
   no overlapping Worker is allowed.
7. If rollback also fails, dsh-work enters a terminal `Failed` state with retryable
...
3. For the profile in the current `Ready` Run context, dsh-work enables plugin
...
4. dsh-work attaches the selected Workspace context to the active session without
...
1. User opens `Settings` from the application menu or system tray.
2. dsh-work opens the separate trusted Settings window on the flat top-level
   `Notifications` route.
3. User turns desktop delivery or an individual notification class on or off.
4. dsh-work persists each switch immediately and applies it to future desktop
...
1. DSH or dsh-work emits a structured action-required, completed, error or enabled
   lifecycle event.
2. dsh-work checks the event identity, current window state and notification
   preferences.
3. If delivery is eligible, dsh-work sends one bounded, localised desktop
   notification.
4. User selects the notification; dsh-work focuses the Workspace or its verified
   target without approving or executing any operation.

## UF-03 — Run a browser task

1. DSH requests a browser operation through the registered dsh-work tool.
2. dsh-work validates the request schema, capability and target scope.
3. If no browser is connected, dsh-work asks the user to enable the browser's personal-agent connection and approve its browser-owned prompt.
4. dsh-work connects to the current profile, then the user assigns the tab AI may control; the existing login state remains available.
...
8. dsh-work returns a structured result to DSH and records a redacted execution event.
...
1. User opens Browser Connection in dsh-work.
2. dsh-work detects supported installed browsers and available connection methods.
3. For supported Chrome, dsh-work guides the user to enable personal-browser remote debugging and the user accepts Chrome's connection prompt.
4. Where auto-connect is unavailable or finer tab scope is preferred, dsh-work offers the signed extension／native-host adapter.
5. User selects an existing tab or asks dsh-work to create a new tab; dsh-work explains that the underlying personal-browser connection may technically access the whole profile.
6. Browser displays its own debugging or extension indicator while control is connected.
7. dsh-work shows browser, profile label, connection method, window and assigned tab; unrelated tabs stay outside the DSH task context.
8. User can disconnect at any time from dsh-work or the browser-owned control surface.

## UF-04 — Startup failure and recovery

1. The Worker exits or fails readiness.
2. dsh-work keeps its native shell responsive and enters `Failed`.
3. dsh-work records diagnostics and offers `Retry`, `Start in safe mode`, `View details` and `Quit` where applicable.
4. Safe mode launches through a separate recovery configuration without deleting the normal configuration.
5. dsh-work explains which optional integration was bypassed and how to return to normal mode.

## UF-05 — Export diagnostics

1. User opens Diagnostics.
2. dsh-work lists the categories to include and flags fields removed by redaction.
3. User previews the generated report.
4. User chooses an explicit destination with the native save dialog.
5. dsh-work confirms the saved path; it never uploads the report automatically.

## UF-06 — Application update or runtime mismatch

1. dsh-work detects that the configured runtime is unsupported or unavailable.
2. The normal launch stops before user data is modified.
3. dsh-work explains what component needs attention and offers only supported remediation paths.
