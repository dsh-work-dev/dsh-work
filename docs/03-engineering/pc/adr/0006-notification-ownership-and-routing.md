# ADR-0006: dsh-work-owned desktop notifications with a structured DSH event bridge
...
dsh-work needs to notify a user when DSH finishes work, waits for a decision or
...
implementation. Those preferences are dsh-work-global and must apply consistently
to both trusted dsh-work windows and the native desktop delivery path.

## Decision

1. dsh-work owns the notification domain, desktop delivery, preference evaluation,
   foreground/background routing and deduplication.
2. DSH remains the owner of in-page notices, conversation context and any
   DSH-specific UI feedback. dsh-work does not hide or rewrite those notices.
3. DSH events enter dsh-work through a versioned structured bridge. The bridge must
...
4. dsh-work persists these initial notification preferences in the existing
...
5. The default policy enables action-required, completed and error desktop
   ...
   surface. DSH contextual feedback remains available regardless of dsh-work
   desktop preferences.
...
7. The first implementation does not add a notification history store or a
   ...
   is retained only for the active dsh-work session until a history use case is
...
- A system notification may not be available or may be disabled by the
  operating system; dsh-work must report delivery failure without changing the DSH
  event outcome.
...
- The Wails notification service is part of a beta dependency and remains
  behind the dsh-work adapter for future replacement.

## Rejected alternatives

- **Take over every DSH web toast:** loses DSH context and couples dsh-work to DSH
...
- **Require a community notification plugin:** plugin lifecycle and delivery
  semantics would be outside dsh-work's versioned ownership, with a high risk of
  duplicate notifications.
- **Add a notification centre immediately:** dsh-work has no persistent chrome over
...
contract, if dsh-work gains a persistent workspace chrome layer, or if users need
cross-session notification history. Any such change must preserve the split
between DSH contextual notices and dsh-work desktop delivery.
