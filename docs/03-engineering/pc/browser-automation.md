...
- Presents available tabs in trusted Work UI; DSH receives only tabs the user assigns.
...
Auto-connect can technically access all profile data surfaced to debugging, including open tabs, storage and cookies. Work policy narrows model-facing operations, but UI must not misrepresent the underlying connection as tab-level technical isolation.

### Work browser extension compatibility adapter
...
- Never sends raw cookies, saved passwords, full browsing history or unrelated-tab contents to Work.

### Native messaging helper

The browser launches a small `work-browser-bridge` executable over the standard length-prefixed stdio protocol. Installers register a manifest separately for supported browsers and platforms.
...
- The helper validates the calling extension origin, then connects to the running Work Host over authenticated local IPC.
- The helper contains no browser policy or task logic and exits when either side disconnects.

### Work Host browser service
...
- Browser connection and tab attachment are revocable from Work and from the extension.
...
An adapter may use Chrome Accessibility, DOM, Input, Page and related protocol domains internally, but the public Work protocol remains semantic.
...
The Host does not show a second approval prompt for an operation already approved through DSH. Browser-owned extension, site-access and debugger warnings remain visible because Work cannot and should not bypass them.
...
- Work may interact with an already authenticated page but must not call cookie or password-store APIs, export credential values or place them in DSH results.
...
- Disconnect or Work exit closes the browser adapter connection and bridge IPC without killing the user's browser.
...
A separate Work-managed profile may remain an explicit fallback for automated tests, privacy-sensitive use or a browser without a supported direct adapter. It is never presented as equivalent to direct user-profile operation and is not the default path.

## Contract tests

Tests cover Chrome auto-connect enable／allow／disconnect, valid and invalid extension IDs, native-host registration, assigned versus unrelated tabs, existing login state, adapter attach／detach, navigation, redirects, stale handles, sensitive fields, DSH approval outcomes, cancellation, browser crash, adapter restart and Work exit. Deterministic tests use local pages; public internet sites are excluded from release gates.
