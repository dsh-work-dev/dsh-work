...
// then disable or remove one while no Worker is running.
...
// because removing the plugin that surfaced it would not help.
...
// DisableFaultPlugin disables one plugin named by the current startup failure
// while the failed Worker has no live PluginManager connection.
func (h *Host) DisableFaultPlugin(ctx context.Context, packageName string) (lifecycle.Status, error) {
	return h.actOnFaultPlugin(ctx, packageName, func(manager PluginFaultManager, launch dshmanager.ResolvedLaunch) error {
		return manager.DisableFaultPlugin(ctx, launch, packageName)
	})
}
